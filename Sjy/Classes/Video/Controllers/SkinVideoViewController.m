 ...
 */
- (void)stopTask {
    if (self.videoControl.session) {
        [self.videoControl.session stopRunning];
    }
...
// 启动相机
- (void)startCamera {
    [self.videoControl.session startRunning];
    
    // 禁止滑动
    self.videoControl.slider.enabled = NO;
    // 全屏按钮
...
// 隐藏摄像头
- (void)hidenCamera {
    if (!self.videoControl.previewLayer.isHidden) {
        self.videoControl.previewLayer.hidden = YES;
    }
}

// 拍照
- (void)takePhoto:(Video *)video {
    if (self.videoControl.previewLayer.isHidden) {
        self.videoControl.previewLayer.hidden = NO;
    }
    
    // 启动摄像头
    // 重置计数
    [[NSUserDefaults standardUserDefaults] setInteger:0 forKey:@"UploadUserLearnTimeCount"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    
    _isCanBack = false;
    
    AVCaptureConnection *conntion = [self.videoControl.imageOutput connectionWithMediaType:AVMediaTypeVideo];
    ...
    [self.videoControl.imageOutput captureStillImageAsynchronouslyFromConnection:conntion completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        // 上传图片
        // 参数 jd:1\2\3
        NSDictionary *parameters = @{ @"type":@"ios", @"jd":M_IP, @"u":video.userId, @"fileinput":UIImageJPEGRepresentation(image, 1.0f) };
        
        [TsyValidateUserBiz ValidateUserWithParameters:parameters success:^(NSArray *array) {
            if (array && array.count > 0) {
                // 获取返回结果
                TsyUserItem *item = array.firstObject;
                
                if (item && [@"1" isEqualToString:item.flag]) {
                    // 图片路径
                    video.imgUrl = item.img;
                    video.bt = item.bt;
                    ...
                    // 设置开始播放时间
                    // video.learnTime;
                    [self setWatchStartTime:[video.learnTime intValue]];
                    
                    NSLog(@"学习时间：%@", video.learnTime);
                    
                    // 开始
                    [self play];
                    
                    // 第一次验证通过后，不上传时间，一分钟后开始上传时间，到第五分钟，暂停视频，上传完时间，完成后再进行验证
                    // 开始上传学习时间
                    if (!self.isBacked) {
                        [self uploadUserLearnTime:video];
                    }
                }
                else {
                    NSLog(@"验证失败：%@", @"失败");
                    ...
                    // 继续拍照，直到验证通过为止
                    if (!self.isBacked) {
                        [self performSelector:@selector(takePhoto:) withObject:video afterDelay:5];
                    }
                }
            }
        } failure:^(NSError *error) {
            if (error) {
                NSLog(@"验证用户信息出错：%@", error.description);
            }
        }];
    }];
    
    [self setCurrentPlaybackTime:[video.learnTime intValue] * 60];
    
    _isSwitching = NO;
}

- (void)uploadUserLearnTime:(Video *)video {
    // 写入本地计数，当计数达到5次后，开始启动相机并开启验证
    NSUserDefaults *defaultUser = [NSUserDefaults standardUserDefaults];
    // 获取计数
    NSInteger timeCount = [defaultUser integerForKey:@"UploadUserLearnTimeCount"];
    
    timeCount = timeCount + 1;
    
    [defaultUser setInteger:timeCount forKey:@"UploadUserLearnTimeCount"];
    
    if (timeCount == 1) {
        // 第一次验证通过后，不上传时间，一分钟后，再上传
        [self performSelector:@selector(uploadUserLearnTime:) withObject:video afterDelay:60];
    }
    else {
        // 第二次开始上传时间
        // 参数
        NSDictionary *parameters = @{ @"u":video.userId, @"c":video.courseId, @"img":video.imgUrl, @"s":video.sessionId, @"bt":video.bt };
        [TsyValidateUserBiz UploadUserLearnTimeWithParameters:parameters success:^(NSArray *array) {
            // 获取结果
            TsyUserLearnInfoItem *item = array.firstObject;
            
            if (item) {
                video.bt = item.bt;
                video.learnTime = item.studytimecount;
            }
            
            // 更新已学习时间
            UILabel *titleLabel = (UILabel *)[self.videoControl viewWithTag:3008];
            
            if (titleLabel) {
                [titleLabel setText:[NSString stringWithFormat:@"您已学习%@分钟，课程最小学习时间为%@分钟！", video.learnTime, video.minLearnTime]];
            }
            
            // 如果学习时间已大于课程最小时间，提示本课程已学完，可以返回
            if ([video.bt intValue] > 20) {
            ...
            if (timeCount == 6) {
                // 计数清零
                [defaultUser setInteger:0 forKey:@"UploadUserLearnTimeCount"];
            ...
            NSLog(@"上传学习时间出错，%@！！！", error.description);
...
- (void)setFullScreen {
    ...
    switch (interfaceOrientation) {
        ...
        case UIInterfaceOrientationPortraitUpsideDown:{ // 电池栏在下
            //			NSLog(@"fullScreenAction第3个旋转方向---电池栏在下");
            [self interfaceOrientation:UIInterfaceOrientationPortrait];
            self.videoControl.previewLayer.connection.videoOrientation = AVCaptureVideoOrientationPortraitUpsideDown;
        }
            break;
        case UIInterfaceOrientationPortrait:{ // 电池栏在上
            //			NSLog(@"fullScreenAction第0个旋转方向---电池栏在上");
            [self interfaceOrientation:UIInterfaceOrientationLandscapeRight];
            self.videoControl.previewLayer.connection.videoOrientation = AVCaptureVideoOrientationLandscapeRight;
        }
            break;
        case UIInterfaceOrientationLandscapeLeft:{ // 电池栏在右
            //			NSLog(@"fullScreenAction第2个旋转方向---电池栏在右");
...
//	NSLog(@"%s", __FUNCTION__);
	...
	if (orientation == UIInterfaceOrientationPortrait && self.isFullscreenMode) {
        ...
        self.videoControl.previewLayer.connection.videoOrientation = AVCaptureVideoOrientationPortrait;
