...
@interface SkinVideoViewControllerView ()<AVCaptureMetadataOutputObjectsDelegate>
...
- (void)layoutSubviews
{
	...
	self.titleLabel.frame = CGRectMake(CGRectGetWidth(self.backButton.bounds), CGRectGetMinX(self.topBar.bounds), 360, CGRectGetHeight(self.topBar.bounds));
    ...
    CGFloat iPhone_Width_Ratio = DeviceWidth/320.0f;
    CGFloat iPhone_Height_Ratio = DeviceHeight/480.0f;
    
    if (DeviceWidth > DeviceHeight) {
        iPhone_Width_Ratio = DeviceHeight/320.0f;
        iPhone_Height_Ratio = DeviceWidth/480.0f;
    }
    
        // lv
    self.previewLayer.bounds = CGRectMake(0, 0, (self.bounds.size.height - 16.0f * iPhone_Width_Ratio)/1.5f , self.bounds.size.height - 32.0f * iPhone_Height_Ratio);
    self.previewLayer.videoGravity = AVLayerVideoGravityResizeAspectFill;
    self.previewLayer.position = CGPointMake(self.bounds.size.width - (self.bounds.size.height - 16.0f * iPhone_Width_Ratio)/3.0f - 16.0f * iPhone_Width_Ratio, CGRectGetMidY(self.bounds));
...
- (AVCaptureVideoPreviewLayer *)previewLayer {
    if (!_previewLayer) {
        ...
        //_previewLayer.borderColor = [UIColor grayColor].CGColor;
        //_previewLayer.borderWidth = 1;
        //start
...
- (PLVSlider *)slider{
	if (!_slider) {
		_slider = [[PLVSlider alloc] initWithFrame:CGRectMake(6, 6, 10, 10)];
